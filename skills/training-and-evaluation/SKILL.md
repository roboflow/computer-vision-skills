...
- **Prerequisites — check both before offering NAS:**
  ...
  2. **Plan entitlement.** NAS is gated on the workspace having both `canTrainNas` and `canTrainNewRFDETR`. Self-serve plans (basic/starter/sandbox/research/trial) need to upgrade; enterprise/legacy need to contact sales. Note this gate is enforced in the training UI, not on the API the MCP calls, so a non-entitled workspace may be able to *start* a NAS run through `trainings_create` without an upfront rejection. Neither flag is readable from the MCP today, so if the user is unsure whether their plan includes NAS, say so rather than asserting it will work. If a run does fail on entitlement, fall back to the recommended named model for the task — `rfdetr-medium` (detection) or `rfdetr-seg-medium` (segmentation) — **not** to a hyperparameter sweep.
